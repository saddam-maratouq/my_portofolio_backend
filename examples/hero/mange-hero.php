...
if (isset($_POST['submit'])) {
  ...
  $title = trim($_POST['title']);
    ...
    $existing_image = $current_data["image_name"] ?? null;
        ...
        $tmpName  = $_FILES['hero_img']['tmp_name'];

         //  pathinfo($imageName, PATHINFO_EXTENSION);
        $fileNameEx = pathinfo($imageName, PATHINFO_EXTENSION);
                                        

        // rare   name 
        $fileName =  'Hero_' .  uniqid() . '.' . $fileNameEx ; 

        $targetPath = "../../upload-img/hero/" . $fileName;

        $filePath = "../../upload-img/hero/";
    ...
    if ($existing_image && file_exists(  $filePath . $existing_image)) {
        ...
        if (!move_uploaded_file( $tmpName, $targetPath)) {
            ...
            header("Location: mange-hero.php?upload_error=1");
            exit();
        }
   ...
   $stmt = $connect->prepare("INSERT INTO $tableName ( `image_name` , `name` , `title`) VALUES (?, ? , ?)");

  // Bind parameters
  $stmt->bind_param("sss", $finalImgName, $myName, $title);
...
// UPDATE version -  updating based on an currentId
$stmt = $connect->prepare("UPDATE $tableName SET `image_name` = ?, `name` = ?, `title` = ? WHERE id = ?");


$stmt->bind_param("sssi", $finalImgName, $myName,  $title , $currentId);

// Execute the statement

    if ($stmt->execute()) {
        
      updateRecordMsg('mange-hero.php',1500);
...
if (isset($_POST['update'])) {
  ...
  $sliderImgName = $_FILES['hero_img']['name'] ?? '';

  $sliderImgName = !empty($sliderImgName) ? $sliderImgName : ($current_data['img_name'] ?? null);

  $existing_image = $current_data["img_name"] ?? null;



  if (!empty($sliderImgName) && $existing_image != $sliderImgName) {

    $tmp_name = $_FILES['hero_img']['tmp_name'];
    $file_name = basename($sliderImgName); // full file name with ext ...


    $target_path = "../../upload-img/main-slider/" . $file_name;

    $filePath = "../../upload-img/main-slider/";
    ...
    if (!move_uploaded_file($tmp_name, $target_path)) {
      // echo "Failed with error: " . error_get_last()['message'] . "<br>";
      // die();
      header("Location: ?upload_error=1");
      exit();
    }
  } else {
    // echo " inside empty exist img ";
    // die();
    $file_name = $existing_image;
  }


  $finalSliderImgName = $file_name ?? null;   //$existing_image check  ; 


  // UPDATE statement (by edit get id .... )
  $stmt = $connect->prepare("UPDATE $tableName SET  `title` = ? ,  `image_name` = ? , `name` = ?     WHERE `id` = ? ");

  $stmt->bind_param("sssi", $title, $finalSliderImgName, $myName, $editId);


  if ($stmt->execute()) {
    updateRecordMsg('view-main-slider.php');
          ...
          <ul class="nav">
            ...
            <li class="nav-item side dropdown alert-warning ">
              ...
              <ul class="dropdown-menu" aria-labelledby="trainingDropdown">
                <li><a class="dropdown-item" href="mange-hero.php">Add </a></li>
                <li><a class="dropdown-item" href="view-hero.php">View All</a></li>
              </ul>
            </li>
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top " id="navigation-example">
        <div class="container-fluid">
          <div class="navbar-wrapper   ">
            <a class="navbar-brand" href="javascript:void(0) text-white  "> <?= $countRows > 0  ? 'Update' : 'Add' ?> Hero
            </a>
          </div>
          ...
          <div class="col-xl-12 col-lg-12">
            ...
            <form action="" method="POST" class="mt-5" enctype="multipart/form-data">
              ...
              <div class="mb-4">
                ...
                <small class="text-muted">Only JPG, PNG, GIF, WEBP allowed</small>
              </div>

              <div class="mb-4">
                <label class="text-white"> Title</label>
                <input type="text" name="title" class="form-control" required
                  value="<?= $countRows > 0 ? htmlspecialchars($current_data['title'] ?? '') : '' ?>">
              </div>

              <div class="mb-4">
                <label class="text-white"> Name</label>
                <input type="text" name="myName" class="form-control" required
                  value="<?= $countRows > 0 ? htmlspecialchars($current_data['name'] ?? '') : '' ?>">
              </div>
