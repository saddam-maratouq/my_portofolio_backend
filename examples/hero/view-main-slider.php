...
$editPageName = "mange-hero.php" ; 



#another way shorthand ....
// ($_GET['id'] ?? 0) > 0 ? deleteRecord($tableName, $_GET['id'], $connect) : null;

# we move code to get file img to delete it from folder target when record deleted  .. 
...
if ( is_numeric($delete_id) &&  $delete_id > 0 ) {
  ...
  $sql = " SELECT `img_name` from  $tableName  where id = $delete_id limit 1  ";
  $result = mysqli_query($connect, $sql);
  $current_data = mysqli_fetch_assoc($result);

  $existingImg = $current_data['img_name'] ?? null ; 
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top " id="navigation-example">
        <div class="container-fluid">
          <div class="navbar-wrapper   ">
            <a class="navbar-brand" href="javascript:void(0) text-white  "> All Sliders  </a>
          </div>
  ...
  <div class="container-fluid">
    <div class="col-xl-12 col-lg-12">
      <!-- Add  Button -->
        ...
        <a href="./mange-hero.php" class="btn ">
  ...
  <table class="table table-dark table-striped table-bordered align-middle text-center w-100">
    ...
    <?php while ($row = mysqli_fetch_assoc($result) ) : ?> 
      ...
      <tr>
        ...
        <td width="50%" ><?= htmlspecialchars($row['description']) ?></td>
       
        <td> <img src="../../upload-img/hero/<?= $row['img_name']?>" alt="blog img"  class="table-img" />  </td>
         <td>
          <a href="<?=$editPageName?>?id=<?=$row['id']?>&edit_data=true" class="text-white me-3" title="Edit">
            <i class="material-icons" >edit</i>
          </a>

          <a href="<?=$urlName?>?id=<?=$row['id']?>" class="text-white" title="Delete you can't undo">
