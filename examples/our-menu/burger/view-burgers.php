...
if ( is_numeric($deleteID) &&  $deleteID > 0 ) {
  ...
  $sql = " SELECT `img_name` from  $tableName  where id = $deleteID limit 1  ";
  $result = mysqli_query($connect, $sql);
  $current_data = mysqli_fetch_assoc($result);

  $existingImg = $current_data['img_name'] ?? null ; 

  $filePath = "../../../upload-img/our-menu/burger/";

  $secFilePath = "../../../upload-img/our-menu/all-meals/"; // sec directory 
    ...
    // ... delete img from sec directory 

    if ($existingImg && file_exists($secFilePath . $existingImg )) {

      unlink($secFilePath . $existingImg );
    }
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top " id="navigation-example">
        <div class="container-fluid">
          <div class="navbar-wrapper   ">
            <a class="navbar-brand" href="javascript:void(0) text-white  "> All Burgers</a>
          </div>
  ...
  <table class="table table-dark table-striped table-bordered align-middle text-center w-100">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td style="max-width: 300px;"><?= htmlspecialchars($row['ingredients']) ?></td>
        <td>$<?= number_format($row['price'], 2) ?></td>
        <td>
          <?php if (!empty($row['img_name'])): ?>
            <img class="table-img"  src="../../../upload-img/our-menu/burger/<?= htmlspecialchars($row['img_name']) ?>" 
                 alt="burger image" 
                 >
          <?php else: ?>
