...
//  $url = '../booking-tables/view-booked.php' 
...
if (isset($_POST['submit'])) {
    ...
    $existing_image = $current_data["image_name"] ?? null;
        ...
        $fileName =  'About_' .  uniqid() . $fileName ; 


        $targetPath = "../../upload-img/about/" . $fileName;

        $filePath = "../../upload-img/about/";
    ...
    if ($countRows === 0 ) {
     ...
     $stmt = $connect->prepare("INSERT INTO $tableName ( `image_name` , `description` ) VALUES (?, ? )");

  // Bind parameters
    $stmt->bind_param("ss" ,    $finalImgName , $description );
...
$stmt = $connect->prepare("UPDATE $tableName SET `image_name` = ?, `description` = ? WHERE `id` = ?");


$stmt->bind_param("ssi", $finalImgName, $description, $currentId);

// Execute the statement

    if ($stmt->execute()) {
        
      updateRecordMsg('mange-about.php',1500);
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top " id="navigation-example">
        <div class="container-fluid">
          <div class="navbar-wrapper   ">
           <a class="navbar-brand" href="javascript:void(0) text-white  "> <?= $countRows > 0  ? 'Update' : 'Add'  ?> About  </a>
          </div>
  ...
  <div class="container-fluid">
    ...
    <div class="mb-4">
            <label class="text-white">Description</label>
             <textarea name="description" class="form-control" rows="5" required> <?= $countRows > 0 ? htmlspecialchars($current_data['description'] ?? '') : '' ?></textarea>
    </div>
